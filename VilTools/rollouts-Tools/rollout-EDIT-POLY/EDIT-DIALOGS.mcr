 ...
 */
function showSmoothingGroupsDialog dialog =
(
	...
	on execute do (
		try (
			...
			if (Filters.Is_EPoly()) then
				showSmoothingGroupsDialog "SmoothingGroupsDialog"
		)
		catch ()
	)
)
...
(
	...
	on execute do (
		try (
			...
			if (Filters.Is_EPoly()) then
				showSmoothingGroupsDialog "MaterialIDDialog"
		)
		catch ()
	)
)
