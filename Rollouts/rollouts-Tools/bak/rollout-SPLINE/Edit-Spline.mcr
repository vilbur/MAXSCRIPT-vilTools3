 ...
 *
 */
macroscript	_spline_get_spline_lenght
category:	"_Edit-Spline"
buttontext:	"Get Length"
toolTip:	"Meassure length of selected spline"
--icon:	"menu:_Context Spline|index:1"
(
	filein(@"C:\scripts\MAXSCRIPT-vilTools3\Rollouts\rollouts-Tools\rollout-SPLINE\Edit-Spline.mcr")
	...
	(
		...
		source_shape = selection[1]

		copy_shape = copy source_shape

		convertToSplineShape copy_shape

		for s = 1 to ( numSplines copy_shape ) do
		(
			spline_length = getSegLengths copy_shape s numArcSteps:100

			append lengths (spline_length[spline_length.count])
		)
		...
		for _length in lengths do length_string += "\n" + _length as string

		messageBox ("Lenghts of splines:\n"+length_string) title:"Length of spline"  beep:false
